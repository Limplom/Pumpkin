...
async fn main() {
    ...
    // The console reader thread may be blocked in `readline` with the terminal
    // in raw mode; restore it before exiting so echo is not left disabled.
    restore_terminal();
...
fn handle_panic(panic_info: &PanicHookInfo<'_>) {
    ...
    if is_main_thread() {
        ...
        // The main thread has panicked and we are about to abort. Restore the
        // terminal first: the console reader thread may be holding it in raw
        // mode, which would otherwise leave input echo disabled (issue #2441).
        restore_terminal();
        exit(1);
    }
